...
describe("Дробь с использованием классов", () => {
    test("создает и нормализует дроби", () => {
        const frac1 = new Fraction(6, 8);
        expect(frac1.numer).toBe(3);
        expect(frac1.denom).toBe(4);

        const frac2 = new Fraction(-6, 8);
        expect(frac2.numer).toBe(-3);
        expect(frac2.denom).toBe(4);
    });
    ...
    test("вычитает дроби", () => {
        ...
        expect(diff.numer).toBe(1);
        expect(diff.denom).toBe(6);
    });
